...
* You may want to make files on your native filesystem available in the container.
By default the host filesystem is not visible inside the container.
You can use the `--volume=` option to `docker run` to mount directories on the host filesystem into the container.
...
* All the previous examples implicitly run `/bin/bash` inside the container.
This is the default but it is also possible to run KLEE directly (useful for scripting) by specifying the command line to use to `docker run`.
