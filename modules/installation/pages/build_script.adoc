...
As example, to build the __STP__ solver, `build.sh` finds the general component information in `v-stp.inc`.
...
For example, for __STP__, it is build in something like `STP_BUILD_PATH="${BASE}/stp-${STP_VERSION}-build"` and installed into `STP_INSTALL_PATH="${BASE}/stp-${STP_VERSION}-install`.
